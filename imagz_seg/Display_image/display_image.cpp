...
double min, max;
    ...
    Mat& ch = channels[2];
    // apply Otsu thresholding (if we need to, we can implement the algo by ourselves)
    Mat bw;
    threshold(ch, bw, 0, 255, THRESH_BINARY | THRESH_OTSU);
    // close small gaps
    Mat kernel = getStructuringElement(MORPH_ELLIPSE, Size(3, 3));
    Mat morph;
    morphologyEx(bw, morph, MORPH_CLOSE, kernel);
    // take distance transform
    Mat dist;
    distanceTransform(morph, dist, DIST_L2, DIST_MASK_PRECISE);
    ...
    for(int idx = 0; idx >= 0; idx = hierarchy[idx][0])
    {
        ...
        // draw the circles
        circle(img, maxLoc, (int)max, Scalar(0, 0, 255), 2);
    }
